<template>
  <div class="space-y-3">
    <div v-if="!ranking" class="bg-gray-900 rounded-xl p-8 text-center text-gray-500 text-sm">
      正在加载健康评分数据...
    </div>
    ...
    <template v-else>
      <div class="grid grid-cols-4 gap-3">
        ...
        <div class="bg-gray-900 rounded-xl p-4 border-l-4 border-orange-500 flex flex-col justify-between">
          <div>
            <div class="text-xs text-gray-400 mb-1">评分历史</div>
            <div class="text-3xl font-bold text-orange-400">{{ store.scoreHistory.length }}</div>
          </div>
          <button v-if="store.scoreHistory.length" @click="showHistory = !showHistory"
            class="text-[11px] mt-2 py-1 px-2 rounded bg-orange-900/40 text-orange-300 hover:bg-orange-900/60 transition-colors self-start">
            {{ showHistory ? '隐藏历史' : '查看历史快照' }}
          </button>
        </div>
      </div>

      <div v-if="showHistory && store.scoreHistory.length" class="bg-gray-900 rounded-xl p-4">
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-sm font-bold text-gray-300 flex items-center gap-2">
            <svg class="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            历史评分快照（共 {{ store.scoreHistory.length }} 条，最近 50 条）
          </h3>
          <div class="flex gap-2 flex-wrap">
            <button v-for="snap in store.scoreHistory.slice(0, 8)" :key="snap.timestamp"
              @click="viewSnapshot(snap)"
              class="text-[10px] px-2 py-1 rounded transition-colors"
              :class="viewingSnapshot?.timestamp === snap.timestamp
                ? 'bg-orange-600 text-white'
                : 'bg-gray-800 text-gray-400 hover:bg-gray-700'">
              {{ formatTime(snap.timestamp) }}
            </button>
            <button v-if="viewingSnapshot" @click="clearSnapshotView"
              class="text-[10px] px-2 py-1 rounded bg-gray-700 text-gray-300 hover:bg-gray-600">
              返回实时
            </button>
          </div>
        </div>
        <div v-if="viewingSnapshot" class="bg-blue-900/20 border border-blue-800 rounded-lg p-3 mb-3">
          <div class="flex items-center gap-2 text-blue-300 text-xs">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>正在查看 <b>{{ formatTime(viewingSnapshot.timestamp) }}</b> 的历史快照数据，点击「返回实时」回到当前评分</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-900 rounded-xl p-4">
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-sm font-bold text-gray-300 flex items-center gap-2">
            ...
            {{ viewingSnapshot ? '历史评分 — 风险排序' : '风险排序（分数越低风险越高）' }}
          </h3>
        ...
        <div class="space-y-2">
          <div v-for="item in displayRanking" :key="item.deviceId"
...
import type { DeviceHealthScore, HealthScoreSnapshot } from '../types'

const store = useModbusStore()
const showHistory = ref(false)
const viewingSnapshot = ref<HealthScoreSnapshot | null>(null)

onMounted(() => {
  console.log('[HealthScorePanel] mounted, devices:', store.devices.length, 'healthRanking:', store.healthRanking)
})
...
const displayRanking = computed<DeviceHealthScore[]>(() => {
  if (viewingSnapshot.value) {
    return viewingSnapshot.value.ranking
  }
  return store.healthRanking?.ranking || []
})
...
function viewSnapshot(snap: HealthScoreSnapshot) {
